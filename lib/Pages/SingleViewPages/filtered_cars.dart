...
class view_filter_cars extends StatefulWidget {
  final String type;
  const view_filter_cars({required this.type,super.key});

  @override
  State<view_filter_cars> createState() => _view_all_carsState();
}

class _view_all_carsState extends State<view_filter_cars> {
  late Future<List<CarDetails>> carDetailsFuture;
  @override
  void initState() {
    super.initState();
    //carDetailsFuture = CarDetailsService().fetchCarDetailsByType(widget.type);
    carDetailsFuture = CarDetailsService().fetchCarDetailsByTypeAPI(widget.type);
    print(widget.type);
  }
